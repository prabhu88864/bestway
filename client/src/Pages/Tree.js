...
export default function BinaryTreeNeoFixed() {
  ...
  const fetchTree = async () => {
    ...
    try {
      const res = await api.get("/api/binary/tree", {
        // if your backend supports depth query, uncomment:
        // params: { depth },
        headers: { Authorization: `Bearer ${token}` },
      });

      setData(res.data);
      if (res?.data?.tree?.userId) setSelectedId(res.data.tree.userId);
    } catch (e) {
      setErr(
  ...
  useEffect(() => {
    if (!loading) setTimeout(centerCanvas, 50);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loading, data, zoom]);

  return (
    <div className="neo-page">
      {/* ✅ CSS without backticks (no syntax errors) */}
      <style>{CSS}</style>
      ...
      <div className="neo-topbar">
        <div className="neo-head">
          ...
          <div className="neo-sub">
            Root: <b>{data?.rootUserId ?? "—"}</b> · Depth:{" "}
        ...
        <div className="neo-actions">
          ...
          <div className="neo-zoom">
            <button className="neo-ic" onClick={zoomOut}>-</button>
            <div className="neo-zoomv">{Math.round(zoom * 100)}%</div>
            <button className="neo-ic" onClick={zoomIn}>+</button>
            <button className="neo-ic ghost" onClick={zoomReset}>R</button>
            <button className="neo-ic ghost" onClick={centerCanvas}>C</button>
          </div>
        </div>
      </div>
...
function NeoNode({ node, selectedId, onSelect }) {
  ...
  return (
    <li className="neo-li">
      <button
        type="button"
        className={"neo-card" + (selectedId === node.userId ? " sel" : "")}
      ...
      >
        ...
        <div className="neo-name">{node?.name || "—"}</div>

        <div className="neo-meta">
          <span className="chip">ID: {node?.userId ?? "—"}</span>
          <span className="chip">{node?.referralCode ?? "—"}</span>
        </div>

        <div className="neo-mini">
          <span>L: {node?.leftUserId ?? "null"}</span>
          <span>R: {node?.rightUserId ?? "null"}</span>
        </div>
      </button>
...
function NeoSlot({ child, side, selectedId, onSelect }) {
  if (!child) {
    return (
      <li className="neo-li">
        <div className="neo-card empty">
          <div className="neo-badge faint">
            <span className="dot" />
            <span className="txt">{side}</span>
          </div>
          <div className="neo-name">Empty</div>
          <div className="neo-mini">No user</div>
        </div>
      </li>
    );
  }
...
/** ✅ CSS as a normal JS string (NO backticks, NO ${} ) */
const CSS =
...
".neo-stage{transform-origin:top center;width:max-content;margin:0 auto;padding:6px 0 20px;}" +
...
".neo-tree{padding-left:0;display:flex;justify-content:center;}" +
".neo-ul{padding-left:0;display:flex;justify-content:center;gap:22px;}" +
".neo-li{list-style:none;text-align:center;position:relative;padding:16px 8px 0 8px;}" +
...
".neo-card{width:168px;min-height:128px;border-radius:18px;padding:12px;border:1px solid rgba(220,235,255,.14);background:linear-gradient(180deg,rgba(18,28,54,.62),rgba(10,16,35,.62));box-shadow:0 18px 44px rgba(0,0,0,.28);color:#eaf0ff;display:flex;gap:10px;flex-direction:column;cursor:pointer;user-select:none;transition:transform .14s ease,box-shadow .14s ease,border-color .14s ease;}" +
...
".neo-name{font-size:16px;font-weight:900;letter-spacing:.2px;text-align:left;}" +
".neo-meta{display:flex;gap:8px;flex-wrap:wrap;}" +
".chip{font-size:12px;font-weight:900;padding:6px 10px;border-radius:12px;border:1px solid rgba(220,235,255,.12);background:rgba(18,28,54,.35);}" +
...
"@media (max-width:520px){.neo-actions{width:100%;justify-content:space-between;}.neo-canvas{height:calc(100vh - 210px);}.neo-card{width:142px;min-height:120px;border-radius:16px;}.neo-ul{gap:14px;}}";
